  ...
  $.fn.birthdaypicker = function( options ) {
    ...
    return this.each(function() {
      ...
      var $fieldset = $("<fieldset class='birthday-picker'></fieldset>"),
          $year = $("<select class='btn btn-primary'  name='birth[year]'></select>"),
          $month = $("<select class='btn btn-primary' name='birth[month]'></select>"),
          $day = $("<select class='btn btn-primary' name='birth[day]'></select>");
      ...
      if (settings["placeholder"]) {
        $("<option value='0' style='color:#000'>Year:</option>").appendTo($year);
        $("<option value='0' style='color:#000'>Month:</option>").appendTo($month);
        $("<option value='0' style='color:#000'>Day:</option>").appendTo($day);
      }
      ...
      for (var i=startYear; i>=endYear; i--) { $("<option style='color:#000'></option>").attr("value", i).text(i).appendTo($year); }
      for (var j=0; j<12; j++) { $("<option style='color:#000'></option>").attr("value", j+1).text(months[settings["monthFormat"]][j]).appendTo($month); }
      for (var k=1; k<32; k++) { $("<option style='color:#000'></option>").attr("value", k).text(k).appendTo($day); }
      ...
      $fieldset.change(function() {
        ...
        if (curMaxDay > actMaxDay) {
          ...
          while (curMaxDay < actMaxDay) {
            curMaxDay++;
            $day.append("<option value=" + curMaxDay + "style='color:#000'>" + curMaxDay + "</option>");
          }
        }
        ...
        if (selectedYear != startYear && curMaxMonth != 12) {
          while (curMaxMonth < 12) {
            $month.append("<option value=" + (curMaxMonth+1) + "style='color:#000'>" + months[settings["monthFormat"]][curMaxMonth] + "</option>");
            curMaxMonth++;
          }
        }
